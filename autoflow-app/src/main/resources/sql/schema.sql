...
(
    ...
    plugin_ids    VARCHAR(32)[],                       -- UUID 数组，存储插件 ID 集合
...
(
    ...
    flow_state    CHAR      DEFAULT CURRENT_TIMESTAMP, -- 耗时（毫秒）
    flow_str      TEXT,                                -- 存储 JSON 数据，推荐使用 JSONB 类型
    creator       VARCHAR(32),                         -- 创建者 ID，使用 UUID 类型
    last_modifier VARCHAR(32),                         -- 最后修改者 ID，使用 UUID 类型
    create_time   TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- 创建时间，默认当前时间
...
(
    ...
    service_id       VARCHAR(32) NOT NULL,                -- 服务节点ID
    loop_id          VARCHAR(32) NOT NULL,                -- 循环ID
    loop_counter     INTEGER     NOT NULL,                -- 循环次数
    nr_of_instances  INTEGER     NOT NULL,                -- 循环实例总数
